    ...
    <div class="h-full overflow-y-auto" style="max-height: calc(100vh - var(--total-nav-height, 120px));">
      <div class="p-4 space-y-4">
        ...
        <div class="border-t pt-3">
          ...
          <button data-cart-video-id="${video.videoId}" onclick="addToCart('${video.videoId}', '${encodeURIComponent(video.title)}', '${encodeURIComponent(video.channel || '')}', '${encodeURIComponent(video.thumbnail || '')}', ${video.views || 0})" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-gray-500 to-gray-600 text-white rounded-xl shadow hover:shadow-lg hover:scale-[1.02] transition-all duration-200 font-semibold text-sm mt-2"><i class="fas fa-cart-plus mr-1"></i> 장바구니 담기 <span class="px-1.5 py-0.5 bg-white/20 rounded-md text-xs font-bold">무료</span></button>
