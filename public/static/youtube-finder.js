  ...
  panelEl.innerHTML = `
    <div class="p-6 space-y-4">
      ...
      <div class="grid grid-cols-3 gap-2 py-3 border-t border-b border-gray-200">
        ...
        <div class="text-center">
          <div class="text-base font-bold text-gray-900">${formatNumber(video.likes)}</div>
          <div class="text-xs text-gray-500">좋아요</div>
        </div>
        <div class="text-center">
          <div class="text-xs font-medium text-gray-900">${new Date(video.publishedAt).toLocaleDateString('ko-KR', { year: '2-digit', month: '2-digit', day: '2-digit' }).replace(/\. /g, '.').replace(/\.$/, '')}</div>
      ...
      <div class="space-y-1.5 text-sm">
        <div class="flex items-center justify-between">
          ...
          <span class="font-semibold text-gray-900">${video.likeRate ? (video.likeRate * 100).toFixed(1) + '%' : ((video.likes / video.views) * 100).toFixed(1) + '%'}</span>
        </div>
        <div class="flex items-center justify-between">
      ...
      <div class="border-t pt-3">
        ...
        <div class="text-sm text-gray-600 leading-relaxed">
          <div id="channel-detail-description" class="overflow-hidden transition-all duration-300" style="max-height: 4.5em;">
            ${video.description || '설명이 없습니다.'}
          </div>
          <button onclick="toggleChannelDescription()" class="text-blue-600 hover:text-blue-700 text-xs mt-2 font-medium">
            더보기
          </button>
        </div>
      </div>
