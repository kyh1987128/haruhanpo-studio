...
- 포맷: 네이버 블로그에 바로 복사-붙여넣기 가능한 순수 텍스트 (HTML 태그 일체 금지, 마크다운 기호 #/##/**/*** 사용 금지)
...
⚠️ 중요 규칙:
- 최종 출력물에는 괄호와 글자 수 표시를 절대 포함하지 마세요.
- #, ##, ###, **, *** 등 마크다운 기호를 절대 사용하지 마세요.
- 반드시 아래의 대괄호 라벨 형식을 사용하세요.
- 각 라벨은 독립된 줄에 작성하고, 라벨 앞뒤로 빈 줄 1개씩 넣으세요.
- 네이버 블로그에 바로 복사-붙여넣기 가능한 순수 텍스트만 출력하세요.

[제목] SEO 최적화 제목 (60자 이내, 메인 키워드 포함)
...
[소제목] 자연스러운 소제목 직접 작성 (예: 렌탈스튜디오의 3가지 매력)
...
[소제목] 자연스러운 소제목 직접 작성 (예: 전문 장비로 퀄리티 UP)
⚠️ "핵심 섹션"이라는 말을 절대 사용하지 마세요.
- 서브 키워드 활용
- 비교 및 분석 내용
- 이모지 3~5개
...
⚠️ 핵심 규칙:
- 캡션 본문은 반드시 500-800자 이내로 작성하세요. 800자를 초과하면 안 됩니다.
- 캡션에는 훅 문장 + 본문 + CTA만 포함하세요.
- 카루셀 기획은 캡션과 완전히 분리된 별도 블록으로 출력하세요.
- 해시태그는 10개 이내로 제한하세요.

1. 캡션 (500-800자 엄수)

[훅 문장 - 첫 2줄 안에 핵심 메시지]
감정/공감/질문으로 시작. 스크롤을 멈추게 하는 첫 문장이 가장 중요.
...
슬라이드 3: [포인트 2]
...
(슬라이드 수는 내용에 맞게 2-10장)

✅ 최종 체크:
✓ 훅 문장이 첫 2줄 안에 있는가
✓ 캡션 본문이 500-800자 이내인가 (800자 초과 금지)
✓ 해시태그 10개 이내인가
✓ CTA가 간결한가
✓ 카루셀 기획이 캡션과 분리되어 있는가`;
}
...
- 반드시 아래의 대괄호 라벨 형식을 사용하세요.
- 각 라벨은 독립된 줄에 작성하고, 라벨 앞뒤로 빈 줄 1개씩 넣으세요.
- 브런치에 바로 복사-붙여넣기 가능한 순수 텍스트만 출력하세요.

출력 형식:
📖 브런치 콘텐츠 (2,000자 이상)

[제목] 감성적이고 궁금증을 유발하는 제목 (15-25자)

[부제목] 제목을 보완하는 한 줄 (20-30자)

[도입부]
독자를 끌어들이는 시작 (300-500자)

[전개부]
이야기의 깊이와 경험 (1,000-1,500자)
