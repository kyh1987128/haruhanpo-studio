...
⚠️ 중요: 최종 출력물에는 괄호와 글자 수 표시를 절대 포함하지 마세요.
예: "## 들어가며 (400-500자)" ❌ → "## 들어가며" ✅
...
⚠️ 플랫폼 차별화 필수:
- 다른 플랫폼과 동일한 내용 반복 금지
- 인스타그램만의 고유한 감성적 관점으로 작성
- 동일한 키워드를 기계적으로 나열하지 말 것
- 핵심 메시지는 하나만 선택하여 깊게 다룰 것
- 비주얼 스토리텔링과 공감을 우선시
...
**유튜브 롱폼 특성:**
- 9-12분 영상 최적화 (풀버전 대본 작성)
- 깊이 있는 스토리텔링
- 시청 유지율 최적화
- 완결성 있는 구성

**🚨 중요: 반드시 9분 30초 분량의 완전한 대본을 작성하세요**

작성 요구사항:
1. 인트로 (00:00-00:30): 강력한 훅
   - **키워드 "${params.keywords}"를 명확히 제시**
   - 이미지의 핵심 요소로 시청자 주목
   - 최소 150단어 분량
   
2. 챕터 1 (00:30-03:00): 문제 제기 / 배경 설명
   - **브랜드 "${params.brand}" 스토리 도입**
   - 이미지와 키워드를 자연스럽게 연결
   - ${params.tone} 톤으로 ${params.targetAge} 타겟 공감 유도
   - 최소 750단어 분량
   
3. 챕터 2 (03:00-06:00): 핵심 내용 / 해결책 제시
   - 구체적인 사례와 근거 제시
   - 이미지 분석 내용을 깊이 있게 활용
   - 최소 750단어 분량
   
4. 챕터 3 (06:00-09:00): 심화 내용 / 추가 가치 제공
   - **이 섹션은 필수입니다. 절대 생략하지 마세요**
   - 브랜드의 차별점과 강점 강조
   - 타겟에게 실질적인 도움이 되는 정보 제공
   - 최소 750단어 분량
   
5. 아웃트로 (09:00-09:30): CTA + 구독 유도
...
**🚨 필수 체크리스트:**
✅ 총 분량: 최소 2,500단어 (9분 30초 분량)
✅ 챕터 3 (06:00-09:00) 반드시 포함
✅ 각 챕터별 최소 단어 수 준수
✅ 타임스탬프 정확히 표기
✅ 중간에 끊지 말고 완결된 대본 작성
...
⚠️ 플랫폼 차별화 필수:
- 다른 플랫폼과 동일한 내용 반복 금지
- LinkedIn만의 비즈니스 인사이트와 전문성 중심
- 데이터와 경험 기반의 깊이 있는 분석
- 실행 가능한 구체적 조언 제공
...
- 감성적이고 문학적인 문체
- 읽는 재미와 여운을 주는 구성
- **🚨 최적 길이: 2,000~3,000자 (긴 글이 브런치의 정체성)**

**🚨 중요: 반드시 2,000자 이상의 완결된 글을 작성하세요**

작성 요구사항:

1. 도입부 (300-500자): 독자를 끌어들이는 시작
   - 질문, 고백, 장면 묘사로 시작
   - 호기심과 공감 유발
   - 이미지와 연결된 첫 인상
   
2. 전개부 (1,000-1,500자): 이야기의 깊이
   - **브랜드 "${params.brand}"와 연결된 개인적 경험**
   - ${params.keywords}를 자연스럽게 녹여내기
   - 감정과 생각의 흐름을 따라가기
   - 구체적인 에피소드와 장면 묘사
   - ${params.tone} 톤으로 ${params.targetAge} 타겟 공감 유도
   
3. 심화부 (500-700자): 통찰과 성찰
   - 경험에서 얻은 깨달음
   - 보편적 의미로 확장
   - 이미지 분석 내용을 깊이 있게 활용
   
4. 마무리 (200-300자): 여운과 메시지
   - 독자에게 남기고 싶은 한 줄
   - ${params.tone} 톤으로 마무리
   - 브랜드와 키워드를 자연스럽게 재강조
